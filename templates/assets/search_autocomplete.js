...
function sync_selection_table(toroot)
{
  ...
  /*      var r = filtered.insertRow(-1);
    ...
    if (gMatches.length > 0) {
        ...
        document.getElementById("no_matches_div").className = "no-display";
...
/*      if (gMatches.length > ROW_COUNT) {
    ...
    if(gMatches.length > 0 && gSelectedIndex < 0){
        set_row_selected(filtered.rows[0], true);
    }
    } else {
        var search = document.getElementById("search_autocomplete");

        document.getElementById("search_filtered_div").className = "no-display";
        document.getElementById("no_matches_div").className = search.value.length > 0 ? "showing" : "no-display"
    }
}
...
function search_changed(e, kd, toroot)
{
    ...
    if (e.keyCode == 13) {
        ...
        if (kd && gSelectedIndex >= 0) {
            ...
            if (HAS_SEARCH_PAGE) {
                ...
                if (gMatches.length > 0){
                    window.location = toroot + gMatches[0].link;
                }
    ...
    // 40 -- arrow down
        ...
        if (search.value.length > 0){
            document.getElementById("no_matches_div").className = "showing";
        }
        gMatches = new Array();
        matchedCount = 0;
        gSelectedIndex = -1;
